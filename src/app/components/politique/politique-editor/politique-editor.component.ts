...
@Component({
  selector: 'app-politique-editor',
  templateUrl: './politique-editor.component.html',
  styleUrls: ['./politique-editor.component.scss'],
})
export class PolitiqueEditorComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.postId) {
      this.blogService
        .getPolitcalPostbyId(this.postId)
  ...
  saveBlogPost() {
    if ($('#science').is(':checked')) {
      ...
      if (this.postId) {
        ...
        this.blogService.updatePoliticalPost(this.postId, this.postData).then(() => {
          this.router.navigate(['/']);
        });
      }
      else{
